...
module.exports = function views(app) {
    ...
    Object.keys(engines).forEach(function (ext) {
        ...
        if (thing.isObject(spec.renderer) && thing.isFunction(module[spec.renderer.method])) {
            ...
            debug('using %s[%s] %s', spec.module, spec.renderer.method, args.length ? 'with arguments' : 'without arguments');
            ...
            debug('using %s[%s]', spec.module, spec.renderer);
            engine = module[spec.renderer];

        } else if (thing.isFunction(module[spec.name])) {
            debug('using %s[%s]', spec.module, spec.name);
            engine = module[spec.name];

        } else if (thing.isFunction(module[ext])) {
            debug('using %s[%s]', spec.module, ext);
            engine = module[ext];

        } else if (thing.isFunction(module.__express)) {
            debug('using %s.__express', spec.module);
            engine = module.__express;
