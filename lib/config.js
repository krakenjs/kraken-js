...
exports.create = function create(options) {
    var deferred, protocols, factory;

    deferred = q.defer();
    protocols = createHandlers(options);

    factory = confit({ basedir: path.join(path.dirname(__dirname), 'config'), protocols: protocols });
    factory.addOverride(path.join(options.basedir, 'config', 'config.json'));
    factory.create(function (err, config) {
        ...
        factory.addOverride(path.join(options.basedir, 'config', config.get('env:env') + '.json'));
        factory.create(function (err, config) {
            if (err) {
                deferred.reject(err);
                return;
