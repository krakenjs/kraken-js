...
exports.compiler = function (srcRoot, destRoot, config) {
    ...
    if (i18n && dust && directory) {
        ...
        options.dust = {
            ...
            precompile: function (context, callback) {
                var data, locality, locale, dest, model;

                data = {
                    fallback: i18n.fallback,
                    ext: '.dust',
                    views: path.join(context.srcRoot, directory)
                };

                // Look for Country/Locale tuple in path
                locale = context.name.match(/(?:([A-Z]{2})\/([a-z]{2})\/)?(.*)/);
                ...
                model = dust.makeBase(data);
                model = model.push({ context: { locality: locality } });

                // onLoad should have been shimmed by i18n, so this should return
                // localized templates.
                dust.onLoad(context.name, model, function (err, data) {
                    if (err) {
                        callback(err);
                        return;
                    }
