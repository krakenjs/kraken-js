...
var proto = {
    ...
    _middleware: function () {
        ...
        // handle forms with formidable
        app.use(function(req, res, next) {
            var formidable = require('formidable'), form = new formidable.IncomingForm();
            form.parse(req, function(err, fields, files) {
                req.fields = fields; // pass along form fields
                req.files = files;   // pass along files

                // remove tmp files after request finishes
                var cleanup = function() {
                    var fs = fs || require('fs'), i;
                    if (files) {
                        for (i in files) {
