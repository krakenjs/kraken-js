...
var proto = {
    ...
    _configure: function (callback) {
        ...
        function next(err) {
            var config, agentSettings, proxySettings;
            ...
            proxySettings = config.get('proxy');
    ...
    _views: function () {
        ...
        app.set('views', this._resolve(viewEngineConfig.templatePath));

        if (i18nConfig) {
            var dusti18n = tryRequire('dustjs-i18n');
            if (dusti18n) {
                i18nConfig.contentPath = this._resolve(i18nConfig.contentPath);
